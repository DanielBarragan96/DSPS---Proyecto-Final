...
static BooleanType FrecuencyFlag = FALSE;

BooleanType get_FrecuencyFlag()
{
	return FrecuencyFlag;
}
...
void FlexTimer3_Init(uint32 OverFlowFrecuency)
{
	uint16 Prescaler = 128;
	uint16 CalculatedMod = 0;
	CalculatedMod = (60000000/(Prescaler * OverFlowFrecuency))- 1;
	...
	FTM3->MOD = CalculatedMod;
	/**Enabling the interrupt,selecting the clock source and a pre-scaler of 128*/
	FTM3->SC = FLEX_TIMER_TOIE| FLEX_TIMER_CLKS_1|FLEX_TIMER_PS_128;
